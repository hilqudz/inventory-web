...
import { addTransaksiMasuk, deleteSupabaseRows, clearSupabaseTable } from '../api';
...
export const TransaksiMasukView: React.FC<TransaksiMasukViewProps> = ({
  records,
  masterItems,
  currentUser,
  onOpenImport,
  ...
  const [documentNo, setDocumentNo] = useState(`IN-2026-${Math.floor(100 + Math.random() * 900)}`);
  ...
  const [remark, setRemark] = useState('Penerimaan Supplier');
  const [qty, setQty] = useState<number | ''>(100);
  const [fromLocation, setFromLocation] = useState('Supplier PT Utama');
  const [toLocation, setToLocation] = useState('Gudang Utama A');
  ...
  const handleSaveRecord = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Try Firebase save (only if quota not exceeded)
      if (!isFirestoreQuotaExceeded) {
        try {
          await addDoc(collection(db, COLLECTIONS.TRANSAKSI_MASUK), {
            postingDate,
      ...
      try {
        ...
        console.warn('Supabase save warning:', spErr);
      }
  ...
  const handleExport = () => {
    ...
    exportToExcel(exportData, 'Transaksi_Masuk_Gudang', 'Transaksi Masuk');
  };
  ...
  const handleDeleteSelected = () => {
    ...
    onRequestDeleteConfirm(
      'Hapus Transaksi Masuk Terpilih',
      `Apakah Anda yakin ingin menghapus ${selectedIds.length} transaksi masuk tercentang?`,
      async () => {
        await deleteSupabaseRows('transaksi_masuk', selectedIds);
        try { await bulkDeleteDocs(COLLECTIONS.TRANSAKSI_MASUK, selectedIds); } catch {}
        setSelectedIds([]);
      },
      false,
      selectedIds.length
  ...
  const handleDeleteAll = () => {
    onRequestDeleteConfirm(
      'Hapus Seluruh Transaksi Masuk',
      'Apakah Anda yakin ingin menghapus SELURUH data Transaksi Masuk?',
      async () => {
        await clearSupabaseTable('transaksi_masuk');
        try { await clearCollectionDocs(COLLECTIONS.TRANSAKSI_MASUK); } catch {}
        setSelectedIds([]);
      },
      true,
      records.length
  ...
  const handleDeleteSingle = (id: string, docNo: string) => {
    onRequestDeleteConfirm(
      ...
      async () => {
        await deleteSupabaseRows('transaksi_masuk', [id]);
        if (!isFirestoreQuotaExceeded) {
          try { await deleteDoc(doc(db, COLLECTIONS.TRANSAKSI_MASUK, id)); } catch {}
        }
      },
      false,
      1
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3.5 rounded border border-slate-200 shadow-xs mb-3">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">
            Penerimaan barang masuk ke gudang. Total {records.length} transaksi.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <button
            onClick={() => {
              setDocumentNo(`IN-2026-${Math.floor(100 + Math.random() * 900)}`);
      ...
      <FilterBar
        ...
        placeholder="Cari berdasarkan Dokumen, Item Code, Remark, Pengirim, Penerima..."
      />
      ...
      <div className="bg-white rounded border border-slate-200 shadow-xs overflow-hidden">
        <div className="px-3 py-2 border-b border-slate-200 flex items-center justify-between bg-slate-100/70">
          <span className="text-[11px] font-bold text-slate-700 uppercase tracking-wider">
            Daftar Transaksi Masuk ({filteredRecords.length})
          </span>
              ...
              {paginatedRecords.length === 0 ? (
                ...
                paginatedRecords.map((r) => {
                  ...
                  return (
                    <tr 
                      key={r.id} 
                      className={`hover:bg-slate-50 transition ${isSelected ? 'bg-emerald-50/40' : ''}`}
                    >
                      ...
                      <td className="p-2 font-mono font-bold text-emerald-600">{r.itemCode}</td>
                      <td className="p-2">
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 border border-slate-100 animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSaveRecord} className="mt-4 space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">DocumentNo *</label>
                  <input
                    type="text"
                    required
                    value={documentNo}
                    onChange={(e) => setDocumentNo(e.target.value)}
                    placeholder="IN-2026-001"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-mono font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Keterangan tambahan penerimaan"
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              ...
              <div className="mt-6 flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2 font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition shadow-sm"
                >
                  {submitting ? 'Menyimpan...' : 'Simpan Transaksi Masuk'}
                </button>
              </div>
