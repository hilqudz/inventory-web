...
interface EmailDoOpenKirimReportModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const EmailDoOpenKirimReportModal: React.FC<EmailDoOpenKirimReportModalProps> = ({
  isOpen,
  onClose,
  spvBreakdown,
  locationBreakdown,
  ...
  const emailSubject = `[LAPORAN DO OPEN KIRIM - APPROVED] Laporan DO Terkirim & Disetujui - ${reportDate}`;

  // Plain Text Version
  const plainTextEmail = `Yth. Manajemen, Audit, & Tim Logistik,

Berikut adalah LAPORAN DO OPEN TERKIRIM / APPROVED per tanggal ${reportDate}:

==============================================================================================================
1. RINGKASAN TOTAL DO OPEN TERKIRIM (APPROVED)
==============================================================================================================
- Total DO Approved        : ${grandTotals.totalDoc.toLocaleString('id-ID', { maximumFractionDigits: 0 })} DO
- Total Line Item          : ${grandTotals.totalRecords.toLocaleString('id-ID', { maximumFractionDigits: 0 })} Baris
- Total Qty Item Approved  : ${grandTotals.totalQty.toLocaleString('id-ID', { maximumFractionDigits: 0 })} Pcs
- Total Nilai Harga Beli   : ${formatCurrency(grandTotals.totalNilaiBeli)}

==============================================================================================================
2. BREAKDOWN DO OPEN TERKIRIM PER AREA SPV
==============================================================================================================
+----+-----------------------------------+-----------+-------------+---------------+-----------------------+
...
==============================================================================================================
3. BREAKDOWN DO OPEN TERKIRIM PER LOKASI / TUJUAN
==============================================================================================================
+----+-----------------------------------+-----------+-------------+---------------+-----------------------+
...
<div style="font-family: Arial, sans-serif; font-size: 13px; color: #1e293b; max-width: 920px; margin: 0 auto; line-height: 1.5;">
  ...
  <div style="background-color: #0f172a; color: #ffffff; padding: 18px 24px; border-radius: 10px; margin-bottom: 20px; border-left: 6px solid #10b981;">
    <div style="font-size: 11px; font-weight: bold; color: #34d399; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 4px;">
      OFFICIAL LOGISTICS REPORT
    </div>
    <h2 style="margin: 0 0 6px 0; font-size: 18px; color: #ffffff;">LAPORAN DO OPEN KIRIM (APPROVED / DISUDAHI)</h2>
  ...
  <p style="margin-bottom: 20px; color: #334155;">Berikut adalah rincian laporan pengiriman <strong>DO OPEN</strong> yang telah disetujui / terkirim (Approved), dikelompokkan berdasarkan Area SPV dan Lokasi Tujuan:</p>

  <!-- Point 1: Global Summary Cards -->
  <h3 style="font-size: 14px; color: #0f172a; border-bottom: 2px solid #10b981; padding-bottom: 6px; margin-top: 24px; margin-bottom: 12px; text-transform: uppercase;">
    1. RINGKASAN TOTAL DO OPEN TERKIRIM (APPROVED)
  </h3>
  <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px; font-size: 12px; border: 1px solid #cbd5e1;">
    <thead>
      <tr style="background-color: #d1fae5; color: #065f46; text-align: left;">
        ...
        <th style="padding: 10px; border: 1px solid #cbd5e1; text-align: center;">Total DO Approved</th>
    ...
    <tbody>
      <tr style="background-color: #ffffff; font-weight: bold;">
        <td style="padding: 12px 10px; border: 1px solid #cbd5e1; color: #0f172a;">Grand Total Barang Terkirim (Approved)</td>
        <td style="padding: 12px 10px; border: 1px solid #cbd5e1; text-align: center; color: #059669; font-size: 13px;">${grandTotals.totalDoc.toLocaleString('id-ID', { maximumFractionDigits: 0 })} DO</td>
  ...
  <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px; font-size: 12px; border: 1px solid #cbd5e1;">
    ...
    <tbody>
      ...
      <tr style="background-color: ${i % 2 === 0 ? '#ffffff' : '#f8fafc'};">
        ...
        <td style="padding: 8px 10px; border: 1px solid #cbd5e1; text-align: center; font-weight: bold; color: #059669;">${spv.docCount} DO</td>
    ...
    <tfoot>
      <tr style="background-color: #e2e8f0; font-weight: bold; color: #0f172a;">
        ...
        <td style="padding: 10px; border: 1px solid #cbd5e1; text-align: center; color: #059669;">${grandTotals.totalDoc} DO</td>
  ...
  <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px; font-size: 12px; border: 1px solid #cbd5e1;">
    ...
    <tbody>
      ...
      <tr style="background-color: ${i % 2 === 0 ? '#ffffff' : '#f8fafc'};">
        ...
        <td style="padding: 8px 10px; border: 1px solid #cbd5e1; text-align: center; font-weight: bold; color: #059669;">${loc.docCount} DO</td>
...
${htmlContent}
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/80 backdrop-blur-xs p-3 overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-5xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden my-auto flex flex-col max-h-[92vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-white flex items-center gap-2">
                <span>Report Email DO OPEN Kirim (Approved)</span>
        ...
        <div className="bg-slate-50 border-b border-slate-200 p-3.5 flex flex-wrap items-center justify-between gap-3 shrink-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {copiedSubject ? <Check className="w-4 h-4 text-white" /> : <Copy className="w-4 h-4 text-emerald-600" />}
            ...
            >
              {copiedHtml ? <Check className="w-4 h-4 text-white" /> : <Sparkles className="w-4 h-4 text-emerald-200" />}
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="px-2.5 py-1.5 text-xs bg-white border border-slate-300 rounded-lg focus:outline-none focus:border-emerald-500 w-48 text-slate-800"
            />
            <button
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-bold rounded-t-lg transition flex items-center gap-1.5 border-t border-x ${
                activeTab === 'preview'
                  ? 'bg-white text-emerald-600 border-slate-300 border-b-transparent shadow-2xs'
